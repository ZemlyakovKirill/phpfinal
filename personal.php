...
<?php
        $query2="select * from opbd6.jobless where username='$_COOKIE[login]'";
        $dbuser='postgres';
        $dbpass='1234';
        $host='localhost';
        $dbname='opbd6';
        ...
        <div id='choiced_item' class='card text-center w-50' style='position:absolute;'>
            ...
            <input id='create_user' class='btn w-25 ml-auto mr-auto' type='submit' value='Обновить' name='update_btn'>
        </div>
        </form>";
        if(isset($_POST['update_btn'])){
            ...
            setcookie("login",$_POST['username'],time()+60*60*24);
            pg_close($db);
            header('Location:account.php');
            }
        }
