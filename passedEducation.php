<?php
    ...
    while($row=pg_fetch_assoc($result)){
        $counter++;
        echo "<p class='card-text'><h6>".$row['name']."</h6> ".$row['startdate']."-".$row['finishdate'] ."(".$row['studentcount']."/".$row['maxquantitystudents'].")"."</p>";
        };
    if($counter==0)
        echo "<p class='card-text'>У Вас нет оконченных групп</p>";
    echo "</div></div>";
    require 'account.php';
?>